...
class MentalStateServices {
    ...
    static async mentalStateHistory(auth, req, res) {
        ...
        // const startDate = moment().subtract(1, 'M').toDate();
        ...
        if (isSub === true) {
            ...
            for (let j = 0; j <= 2; j++) {
                ...
                const startDate = moment().subtract(substractNumber, 'months').startOf('month').toDate();
                const endDate = moment().subtract(substractNumber, 'months').endOf('month').toDate();
                ...
                for (let i = startDate; i <= endDate; i = moment(i).add(1, 'day').toDate()) {
                    ...
                    newdate.push({ date: moment(i).unix(), emoji: findLast3Week ? findLast3Week.emojiId.emoji : "" })
                }

                dates.push({ month: moment(startDate).month() + 1, data: newdate })
                substractNumber++;
            }
